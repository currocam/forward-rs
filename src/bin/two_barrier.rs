...
#[derive(Parser)]
#[command(about = "Two-locus barrier model")]
struct Args {
    ...
    selection_coeff: f64,
    #[arg(long, default_value = "two_barrier.trees")]
    output: String,
}
...
fn main() -> Result<()> {
    ...
    let barrier2 = Locus {
        position: seq_len / 3.0,
        selection_coeffs: vec![
            1.0,                   // strongly beneficial
            -args.selection_coeff, // island deleterious
        ],
    };
    let loci: Vec<Locus> = vec![barrier1, barrier2];
